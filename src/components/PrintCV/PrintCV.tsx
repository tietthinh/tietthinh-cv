...
const SectionTitle: React.FC<{ children: React.ReactNode }> = ({
  ...
  <h2 className="mb-1 mt-3 border-b border-neutral-300 pb-0.5 font-display text-[12px] font-bold uppercase tracking-[0.12em] text-crimson">
    {children}
  </h2>
);
 ...
 * avatar and condenses every section to fit on one or two pages.
 */
export const PrintCV: React.FC<PrintCVProps> = ({
  ...
  return (
    <div className="hidden text-[11px] leading-snug text-neutral-800 print:block">
      {/* Header */}
      <header className="mb-3 flex items-center gap-4 border-b-2 border-crimson pb-3">
        <Image
          src={avatarUrl}
          alt={name}
          width={88}
          height={88}
          className="h-[84px] w-[84px] shrink-0 rounded-md border border-neutral-300 object-cover"
          priority
        />
        <div className="min-w-0">
          <h1 className="font-display text-[26px] font-bold leading-none text-neutral-900">
            {name}
          </h1>
          <p className="mt-1 text-[12px] font-semibold text-crimson">
            Senior Software Engineer · Frontend &amp; Full-stack
          </p>
          <div className="mt-1.5 flex flex-wrap gap-x-3 gap-y-0.5 text-[10px] text-neutral-600">
            <span>{email}</span>
          ...
          {groups.map((group) => (
            <div key={group.company} className="break-inside-avoid">
              <div className="flex items-baseline justify-between gap-2">
                <h3 className="font-display text-[12px] font-bold text-neutral-900">
                  {group.company}
                </h3>
              ...
              <ul className="ml-3.5 mt-0.5 list-disc space-y-0.5 marker:text-crimson">
                {group.items.map((project) => (
                  <li key={project.name} className="text-[10.5px] text-neutral-700">
